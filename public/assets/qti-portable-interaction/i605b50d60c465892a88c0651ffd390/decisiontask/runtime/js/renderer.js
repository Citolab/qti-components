  ...
  function renderGame(id, $container, config, responseObj, assetManager, timer) {
    ...
    function exitgame() {
      ...
      setTimeout(function () {
        $container
          .trigger('pcidone')
          .find('.globalWrapper')
          .html("<div class='endmsgcontainer'>Cliquer sur Suivant</div>");
      }, 1000);
    }
    ...
    if (tlimit > 0) {
      var timeinterval = setInterval(function () {
        ...
        if (t.millisec <= 0) {
          ...
          // Push to data
          dataout.push({
            stimulusindex: datain[i]['stimulusindex'].trim(),
            stimulus: datain[i]['stimulus'].trim(),
            time: duration,
            correct: correct
          });
          responseObj.base.string = JSON.stringify(dataout);
    ...
    if (level == 2 && respkey == true) {
      ...
      $(document).keydown(function (e) {
        switch (e.which) {
          case 81: // key Q
            if (keyisdown['81'] == null && timeoutdone) {
              // first press
              ...
              if (feedback == true) {
                ...
                $container
                  .find('.feedbackcontainer')
                  .append(
                    "<audio autoplay class='feedbackaudio' src= " +
                      assetManager.resolve('decisiontask/runtime/assets/feedback' + nbconsecutivecorrect + '.wav') +
                      '></audio> '
                  );
              }
  ...
  function renderItem(id, $container, config, responseObj, assetManager, timer) {
    ...
    $container.find('.startbutton').click(function () {
      ...
      $container
        .find('.globalWrapper')
        .html(
          "<div class='itemWrapper'><div class='stimuluscontainer'></div><div class='buttoncontainer'></div></div>"
        );
