  ...
  var self = {
    ...
    _init: function (dom, configuration, state) {
      ...
      if (
        ...
        this._state.revealed.length === images.length
      ) {
      ...
      for (var i = 0; i < images.length; i++) {
        ...
        image.outerHTML =
          "<button class='hmh-tap-image' aria-live='polite' aria-relevant='all'>" +
          source +
          "</button>";
      }
    ...
    onclick: function (event) {
      ...
      this._state.revealed[img.dataset.index] = toggle
        ? !this._state.revealed[img.dataset.index]
        : true;
    },

    _swap: function (img) {
      var toggle =
        this._props.toggle && "true".localeCompare(this._props.toggle) === 0;
