...
const dragAndWaitForPlacement = async (
  ...
  const draggableIdentifier = draggable.getAttribute('identifier');
  const draggableTag = draggable.tagName.toLowerCase();

  const resolveDraggable = (): Element => {
    if (draggable.isConnected) {
      return draggable;
    }

    if (!draggableIdentifier) {
      return draggable;
    }
...
export const NoCorrectResponse: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    const itemShowCorrect = correctionCanvas(canvasElement).querySelector('item-show-candidate-correction');
...
export const GapMatch: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    const canvas = within(correctionCanvas(canvasElement));
    const assessmentItem = await getAssessmentItemFromItemContainer(correctionCanvas(canvasElement));
...
export const Order: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step('Verify candidate correction states on placed choices', async () => {
      ...
      // Marking is by longest increasing subsequence, not strict position: the largest set already
      // in the right *relative* order counts as correct. Correct order is C, A, B and the response
      // is C, B, A — so C-then-B is a valid run and only DriverA falls outside it. One misplaced
      // chip does not cascade into everything after it being marked wrong.
      expect(drop0Choice.internals.states.has('candidate-correct')).toBe(true);
      expect(drop1Choice.internals.states.has('candidate-correct')).toBe(true);
