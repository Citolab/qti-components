...
export const Test2: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    const showCorrectButton = await findByShadowText(correctionCanvas(canvasElement), 'Show correct response');
    await waitFor(() => expect(showCorrectButton).toBeEnabled());

    // check the first alernative
    const firstAlternative = await findByShadowText(correctionCanvas(canvasElement), 'Richting het noorden');
    ...
    const secondAlternative = await findByShadowText(correctionCanvas(canvasElement), 'bipolaire staat');
    ...
    await await findByShadowText(correctionCanvas(canvasElement), 'bipolaire staat');
    await fireEvent.click(prevButton);
...
export const Test: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await getAssessmentItemFromTestContainerByDataTitle(correctionCanvas(canvasElement), 'Info Start');
    ...
    const firstItem = await getAssessmentItemFromTestContainerByDataTitle(
      correctionCanvas(canvasElement),
      'Richard III (Take 3)'
    );
...
export const TestFullCorrectResponse: Story = {
  ...
  `,
  play: async ({ canvasElement }) => {
    const qtiTest = correctionCanvas(canvasElement).querySelector('qti-test') as unknown as {
