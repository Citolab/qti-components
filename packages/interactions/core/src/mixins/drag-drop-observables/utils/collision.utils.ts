...
export function closestCornersWithInventoryPriorityCollision(
  ...
  // Keep disabled droppables in collision resolution.
  // They remain non-droppable via allowDrop(), but this lets drag-end detect
  // "attempted drop on disabled target" and trigger proper invalid/max messaging.
  const regularDroppables = allRegularDroppables;
  ...
  if (closestDragContainer && closestDroppable) {
    ...
    if (minDragContainerDistance <= minDroppableDistance * inventoryPriorityThreshold) {
      return closestDragContainer;
    }
