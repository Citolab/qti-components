...
- **DragDropSlottedSortableMixin**: Enhances slotted interactions with in-slot reordering capabilities

## Mixin Composition

```
DragDropCoreMixin (base)
    ├── DragDropSlottedMixin (inventory → slot)
    │       └── DragDropSlottedSortableMixin (adds in-slot reordering)
    └── DragDropSortableMixin (standalone reordering)
```
...
class MyInteraction extends DragDropSlottedSortableMixin(
  ...
  // Items can be swapped between slots (cross-slot swapping)
}
```
...
// Disable cross-slot swapping
this.allowCrossSlotSwap = false;

// Custom sorting strategy (vertical or horizontal)
this.sortingStrategy = new HorizontalListSortingStrategy();
...
|----------|------|---------|-------------|
| `allowReorder` | `boolean` | `true` | Enable/disable in-slot reordering |
| `allowCrossSlotSwap` | `boolean` | `true` | Enable/disable cross-slot item swapping |
| `sortingStrategy` | `SortingStrategy` | `VerticalListSortingStrategy` | Strategy for calculating sort positions |
| `sortablePlaceholderConfig` | `PlaceholderConfig` | `{}` | Styling for the drop placeholder |
