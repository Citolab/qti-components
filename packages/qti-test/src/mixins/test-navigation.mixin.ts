...
export const TestNavigationMixin = <T extends Constructor<TestBaseInterface>>(superClass: T) => {
  abstract class TestNavigationClass extends superClass implements ITestNavigationMixin {
    ...
    private _isLinearNavigationRestricted(type: 'item' | 'section', targetId: string): boolean {
      ...
      const navigationMode = (section?.navigationMode ||
      ...
      const allItemRefs = Array.from(
        this._testElement?.querySelectorAll('qti-assessment-item-ref') || []
      ) as QtiAssessmentItemRef[];
