...
export const LinearNavigation: StoryObj = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    // The item ref only exposes its assessmentItem once the item has rendered.
    // test-end-attempt reaches the candidate's item through that reference, so
    // clicking before it resolves does nothing at all.
    async function waitForActiveItem() {
      await waitFor(() => {
        const test = canvasElement.querySelector('qti-test') as unknown as {
          sessionContext?: { navItemRefId?: string };
        };
        const navItemRefId = test?.sessionContext?.navItemRefId;
        const itemRef = canvasElement
          .querySelector('test-container')
          ?.shadowRoot?.querySelector<
            Element & { assessmentItem?: unknown }
          >(`qti-assessment-item-ref[identifier="${navItemRefId}"]`);
        if (!navItemRefId || !itemRef?.assessmentItem) throw new Error('active item is not rendered yet');
      });
    }
