...
Implementation contract lives at `packages/qti-theme/CONTRACT.md`.
Use this file for design rationale and migration context.
...
| Styling a dropped chip       | **`part="drag"`** on the clone. A **bare `::part(drag)`** reaches it in _any_ target's shadow root (§7 #5), and composes with states (§7 #3). `:state(dropped)` is unnecessary.                                                                                                                                                         |
...
| Drag/drop states             | **`dragging`** and **`placeholder`** on the _source_ chip in the light-DOM bank (§9). "Dropped" is structural, not a state.                                                                                                                                                                                                             |
...
So where the chip _ends up_ depends on where the droppable lives. And that differs per
interaction:
...
## 2. `ch` / `cha` — the naming problem is real, but it's a _role_ problem
...
inside it**. What it _depicts_ changes with role:
...
You are right that `ch`/`cha` are bad names. But note the fix is _not_ to rename them to
...
qti-simple-choice:state(radio)::part(control) {
  border-radius: 50%;
}
qti-simple-choice:state(checkbox)::part(control) {
  border-radius: 0.25rem;
}
qti-simple-choice[qti-draggable]::part(control) {
  mask: var(--qti-grip-icon);
}
```
...
it survives the drag case (a grip _is_ the interactive control of a chip), and it reads well
next to `:state(radio)`.
...
**Important:** `exportparts` must be set on the _clone_, in `dropDraggableInDroppable`, since
...
| gap-match         | `drops` (slot)                                             | _(none — `qti-gap`, light DOM)_                             | light  |
| match             | —                                                          | _(none — `qti-simple-associable-choice`'s `dropslot` slot)_ | light  |
| graphic-gap-match | —                                                          | _(none — `qti-associable-hotspot`, light DOM)_              | light  |
...
- **`data-has-drop` is asymmetric.** It is _set_ only inside the `QTI-SIMPLE-ASSOCIABLE-CHOICE`
  branch of `dropDraggableInDroppable` (`drag-drop-slotted.mixin.ts:516`) but _removed_ generically
...
Naming for the parts that _do_ stay (shadow containers):
...
- `dropslot` → `drop` as well (match's slot _is_ the target).
...
`::part(control radio checked)` is a _different name for the same part_, matched by subset — not
...
the part's _own_ element. Match's cells are plain `<span>`s in the interaction's shadow root:
...
| `marker`       | `qti-simple-choice`       | the order-number badge                                  | _(none — unstylable)_                                       |
| `prompt`       | every interaction         | the `qti-prompt` region                                 | _(only choice-interaction)_                                 |
| `message`      | every interaction         | validation message                                      | _(missing on order, slider)_                                |
| `drags`        | dnd interactions          | source container                                        | `drags` ✅                                                  |
| `drops`        | dnd interactions          | drop-target container                                   | `drops`, `drop-container`                                   |
| `drop`         | **every** dnd interaction | one drop target, shadow-internal, containing a `<slot>` | `drop-list`, `dropslot`, `associables-container`, _(none)_  |
...
by `:state(dragging)` / `:state(placeholder)`, and the two places that _read_ `style.opacity` back as
...
| 5   | Does a **bare** `::part(drag)`, no element on the left, match?         | ✅ **Yes.** One selector reaches a chip in _any_ drop target's shadow root. |
...
_not_ a substitute for parts across a shadow boundary — they only unify light-DOM chips.
...
### Custom properties are _not_ required — a correction
...
`qti-simple-choice.styles.ts`) — and therefore that the token vocabulary had to be frozen _before_
Stage B.

Wrong. Result #5 means the theme reaches the chip perfectly well through `::part(drag)`. Order and
associate already prove it: their dropped chips live in the interaction's shadow and _are_ themed,
...
Today the grip glyph is drawn by reaching _through_ the chip into its shadow:

```css
:state(drag)::part(control)::before {
  /* grip */
}
```
...
**Option B — all dropped chips live in the interaction's shadow root.** ← _requested_
...
Option B as literally requested (drops in shadow) gives the cleanest _encapsulation_ and the
...
moving the _target_ into shadow and letting the chip stay light.

---

## 8b. Styling a chip that sits in a drop container

> **Superseded by §8c.** This section analysed the _slotted_ model. The adopted model renders
...
work**, and the reason is worth internalising: a _slotted_ element is not in the shadow tree. It
...
If instead the target _renders_ the chip into its shadow root (§8c), the chip is in the shadow
...
`placeholder` rather than `placed`: the state sits on the chip that _stayed behind_ and is now
...
| `qti-gap-match-interaction`         | `qti-gap-text`                 | `<qti-gap>`                                       | ✅ custom element    | **light** — authored _inline in a sentence_  | _(none)_                                | —                                                    |
| `qti-match-interaction`             | `qti-simple-associable-choice` | `<qti-simple-associable-choice>` (last match-set) | ✅ custom element    | **light** — authored                         | `dropslot` (a slot in its _own_ shadow) | `data-has-drop`                                      |
| `qti-graphic-gap-match-interaction` | `qti-gap-img, qti-gap-text`    | `<qti-associable-hotspot>`                        | ✅ custom element    | **light** — authored, positioned on an image | _(none)_                                | —                                                    |
...
Its position _in the prose_ is its meaning. The same holds for `qti-associable-hotspot`
...
The dropped chip then always lives inside _a_ shadow root — the **target's**, not the
...
**Adopted:** the drop target renders its contents from _data_, in its own shadow root, via the
...
html`<div part="drops">
  ${this.dropLists.map(
    list =>
      html` <div part="drop" identifier=${list.id}>
        ${repeat(
          list.drags,
...
| response is computed by    | `collectResponseData()` → `querySelectorAll`          | it _is_ the state; DOM is derived          |
...
§8b concluded that `part="dragged"` cannot work, because a _slotted_ chip stays in the light
tree. Under declarative rendering the chip is **not slotted** — it is rendered _inside the
target's shadow root_. So it is in the shadow tree, and `part` applies:

```css
qti-gap::part(drag) {
  border: none;
} /* the dropped chip, borderless */
qti-gap::part(drag)::part(control) {
  /* ✗ invalid — parts don't chain */
}
qti-gap::part(control) {
  /* ✓ via exportparts="control" on the chip */
}
```
...
`:state(placeholder)`, which live on the _source_ chip in the light-DOM bank.
...
This is not a blocker, it is the _point_: those functions exist only because the DOM was the
...
`Node` by reference, so a stable node is _moved_, never recreated. Key the `repeat()` by
`identifier`.
...
Inverting it with `@lit/context` solves this: the interaction _provides_ reactive drag-drop
state; each drop target _subscribes_ and pulls the slice it needs. Nobody queries anybody.
...
| **In-place mutation of the context object does _not_ notify consumers**                                                                               | ✅ (silently does nothing) |
| A consumer that upgrades _before_ its provider still connects, given a `ContextRoot`                                                                  | ✅                         |
...
this. Today the parent _pushes_ the role in, imperatively, after finding the children by query:
...
smell as §8c/§8d. The fix is the same shape — the parent _provides_, the child _subscribes_:
...
### The context must be provided by the _interaction_, not by `qti-assessment-item`
...
_optionally_ (`this.ctx?.…`), so an interaction with no item above it still functions. The two new
...
1. **`min-width` reaches only two of five interactions.** `applyDropzoneAutoSizing` sets it when
   ...
   merely _look_ sized because they are `flex: 1`.
...
So Kennisnet resorts to matching the _inline style string_:
...
Worse, the mixins _read the styles back as if they were state_:
...
the _legacy_ `drag-drop-interaction-mixin.ts:1028`, and on the floating clone, not the source.
...
| _(nothing)_                                          | `qti-droppable="true"` — the light-DOM counterpart of `qti-draggable` |
