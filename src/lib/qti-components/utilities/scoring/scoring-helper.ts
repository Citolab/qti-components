...
export class ScoringHelper {
  public static compareSingleValues(
    value1: string,
    value2: string,
    baseType: BaseType
  ): boolean {
    switch (baseType) {
      ...
      case "integer": {
        ...
        if (!isNaN(int1) && !isNaN(int2)) {
          ...
          console.error(
            `Cannot convert ${value1} and/or ${value2} to int.`
          );
        }
        break;
      }
      case 'float': {
        ...
        if (!isNaN(float1) && !isNaN(float2)) {
          ...
          console.error(
            `couldn't convert ${value1} and/or ${value2} to float.`
          );
        }
        break;
      }
      ...
      case 'directedPair': {
        ...
        if (pair1.length === 2 && pair2.length === 2) {
          ...
          console.error(
            `compared two pair but one of the values does not have 2 values: 1: ${value1} 2: ${value2}`
          );
        }
        break;
      }
