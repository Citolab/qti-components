...
async function expectNavTo(qtiTest: QtiTest, navItemRefId: string) {
  // Fetching and transforming the test doc can outrun poll's 1s default when
  // the whole suite is running, so allow the same 5s the button polls use.
  await expect.poll(() => qtiTest.sessionContext.navItemRefId, { timeout: 5000 }).toBe(navItemRefId);
...
/**
 * The active item ref only exposes its assessmentItem once the item has
 * rendered. test-end-attempt reaches the candidate's item through that
 * reference, so a click before it resolves does nothing at all.
 */
async function endAttempt(qtiTest: QtiTest, endAttemptBtn: HTMLElement) {
  const testContainer = qtiTest.querySelector('test-container');
  await expect
    .poll(() =>
      Boolean(
        testContainer?.shadowRoot?.querySelector<{ assessmentItem?: unknown } & Element>(
          `qti-assessment-item-ref[identifier="${qtiTest.sessionContext.navItemRefId}"]`
        )?.assessmentItem
      )
    )
    .toBe(true);
...
describe('navigation-mode', () => {
  ...
  describe('Linear', () => {
    ...
    test('should allow moving to next item but still disable Prev button', async () => {
      ...
      await endAttempt(qtiTest, endAttemptBtn);
