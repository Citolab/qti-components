...
Each control draws an icon button with an English accessible name; slotted content replaces the icon, and a slotted `<svg>` is sized like the default. `label` (and `pause-label` on `<test-tts-play>`) names the button — set it when you slot an icon of your own, which has no text to be named by. The buttons read the qti-theme tokens, with the theme's defaults as fallbacks, and `--test-button-size` / `--test-tts-icon-size` resize them.
